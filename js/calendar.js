(function(root, factory) {
    ...
    var defaults = {
        ...
        SUBMIT = '[' + ACTION_NAMESPACE + 'subimt]',
        BACK = '[' + ACTION_NAMESPACE + 'back]',

        CLOSEBTN = '[' + ACTION_NAMESPACE + 'closeBtn]',
        ...
        TEMPLATE = [
            '<div class="calendar-inner">',
                '<div id="titleText"><div class="setDateText"><b>设置可售日期</b></div><div class="closeBtn" data-calendar-closeBtn>X</div></div>',
            ...
            '<button data-calendar-subimt id="sub">确认预售</button>',
            '<button data-calendar-back id="back">返回今天</button>',
            '</div>',
    ...
    function Calendar(element, options) {
        ...
        //Benson 4
        this.times2 = this.options.times2;
    }
    ...
    Calendar.prototype = {
        ...
        selectedDay: function(d, type) {
            ...
            var arr = this.getDisDateValue(),
                ...
                toggleClass = function() {
                    var s = this.$dateItems.children(':eq(1)').find('[' + ITEM_DAY + ']:not(.' + NEW_DAY_CLASS + ', .' + OLD_DAY_CLASS + ')').filter(function(index) {
                        return parseInt(this.innerHTML) === d;
                    });
                    if(s.attr('class').indexOf(SELECT_CLASS)!=-1){
                        s.removeClass(SELECT_CLASS);
                        flages = 'rrrr';
                    }else{
                        s.addClass(SELECT_CLASS);
                        flages = 'add';
                    }
                };
            if (type) {
            ...
            if(flages=='add'){
                ...
                for(var i=0;i<this.times2.length;i++){
                    if(this.times2[i]==t){
                        this.times2.splice(i,1);
                    }
                }
            }
        ...
        event: function() {
            ...
            _this.$element.on('click', SUBMIT, function() {
                //这里ajax提交东西
                alert(_this.times2);
                //关闭插件
                _this.$element.hide();
            })
            _this.$element.on('click', CLOSEBTN, function() {
                //关闭插件
                _this.$element.hide();
            })
            //回到今天
            _this.$element.on('click',BACK,function(){
                times=[];
                var y = 2017;
                _this.updateDateView(y);
                vc('date', y);
